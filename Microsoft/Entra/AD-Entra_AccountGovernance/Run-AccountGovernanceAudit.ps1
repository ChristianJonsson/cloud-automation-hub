...
#           Runs scripts 03-06 in sequence, sharing a single timestamped output
#           directory across all steps and writing a run manifest on completion.
#
# Usage:
#   .\Run-AccountGovernanceAudit.ps1
...
param(
    ...
    # Skip script 04 (AD user export). Useful when AD files were transferred
    # manually from a domain-joined machine.
    [switch]$SkipADExport,

    # Skip script 06 (NDJSON to JSON conversion). Use when only NDJSON output
...
# --- Step 04: AD user export (one per forest) ---------------------------------
if (-not $SkipADExport) {
    foreach ($forestName in $forestsToRun) {
        Write-Log "--- Step 04: AD user export for forest '$forestName' ---"
        try {
            . "$PSScriptRoot\04_ExportADUsers.ps1" -ForestName $forestName
        } catch {
            Write-Log "Step 04 FAILED for forest '$forestName': $_"
            throw
        }
    }
} else {
    Write-Log "--- Step 04: Skipped (SkipADExport) ---"
}

# --- Step 05: Cross-reference -------------------------------------------------
Write-Log "--- Step 05: Cross-reference ---"
try {
    . "$PSScriptRoot\05_CrossReference.ps1"
} catch {
    Write-Log "Step 05 FAILED: $_"
    throw
}

# --- Step 06: Convert NDJSON to JSON ------------------------------------------
if (-not $SkipConvertToJson) {
    Write-Log "--- Step 06: Convert NDJSON to JSON ---"
    try {
        . "$PSScriptRoot\06_ConvertToJson.ps1"
    } catch {
        Write-Log "Step 06 FAILED: $_"
        throw
    }
} else {
    Write-Log "--- Step 06: Skipped (SkipConvertToJson) ---"
}
...
$manifest = [ordered]@{
    ...
    SkippedSteps      = @(
        ...
        if ($SkipADExport)      { "04_ExportADUsers" }
        if ($SkipConvertToJson) { "06_ConvertToJson" }
    )
}
