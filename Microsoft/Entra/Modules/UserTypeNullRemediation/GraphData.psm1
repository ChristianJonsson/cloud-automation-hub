...
function Test-CachedUsersData {
    ...
    if ($null -eq $CandidateUsers) {
        return [pscustomobject]@{
            IsReusable = $false
            Reason = 'Cached users variable is null'
            Users = @()
        }
    }
    ...
    if ($userArray.Count -eq 0) {
        return [pscustomobject]@{
            IsReusable = $false
            Reason = 'Cached users variable is empty'
            Users = @()
        }
    }
    ...
    if ($null -eq $firstUser) {
        return [pscustomobject]@{
            IsReusable = $false
            Reason = 'First cached user entry is null'
            Users = @()
        }
    }
...
function Test-CachedVerifiedDomainsData {
    ...
    if ($null -eq $CandidateDomains) {
        return [pscustomobject]@{
            IsReusable = $false
            Reason = 'Cached verified domains variable is null'
            Domains = @()
        }
    }
    ...
    $normalizedDomains = @($rawDomains | ForEach-Object { "$_".Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
